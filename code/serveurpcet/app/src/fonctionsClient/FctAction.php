...
class FctAction {
	...
	public static function modifierBudget($idAction, $nouveauBudgetTotal, $nouveauBudgetConsomme, $loginUtilisateur){
		...
		if($ancienBudgetTotal != $nouveauBudgetTotal && $ancienBudgetConsomme != $nouveauBudgetConsomme){
			$nbModif = 2;
			$description = "Ancien budget total : ".$ancienBudgetTotal." - Nouveau budget total : ".$nouveauBudgetTotal." - Ancien budget consommé : ".$ancienBudgetConsomme." - Nouveau budget consommé : ".$nouveauBudgetConsomme;
			...
			$nbModif = 1;
			$description = "Ancien budget consommé : ".$ancienBudgetConsomme." - Nouveau budget consommé : ".$nouveauBudgetConsomme;
		}
		if($nbModif != 0){
	...
	public static function verifModifAction($tableauParametres){
		...
		$estModifie=false;
			// Pour chaque partenaire de l'action presente, on check si les partenaires ont changé
	...
	public static function modifierAction($idUtilisateur,$tabParamVerifies){
		...
		$dateModif = date('Y-m-d H:i:s');
		$typeModif = "Modification d'une action";
	...
	public static function calculerDateDeFin($phases) {
		...
		if ($dateFin == null) {
			$dateFin = "Aucune date de fin prévue";
		}
		return $dateFin;
	}
	
	/*Calcul la date de début d'une action en fonction des dates de début de ses phases*/
	public static function calculerDateDeDebut($phases) {
		...
		if ($dateDebut == null) {
			$dateDebut = "Aucune date de début prévue";
		}
		return $dateDebut;
	}
	...
	public static function calculerAvancement($phases) {
		$nbPhases = 0;
		// On consid�re que les phases qui n'ont pas d'�tat (null ou  vide "") sont non d�mar�es
		$nbNonDemaree = 0;
		$nbSuspendues = 0;
		$nbEnCours = 0;
		$nbEnProjet = 0;
		...
		// Calcul des nombre d'�tats par type d'�tat
		foreach($phases as $phase) {
			$nbPhases++;
			$etatCourant = $phase->avancement_phase;
			if ($etatCourant == null || $etatCourant == "" || $etatCourant == "Non démarrée") {
				$nbNonDemaree++;
			}
