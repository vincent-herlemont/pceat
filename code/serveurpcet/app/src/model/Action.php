...
public static $nameTable = "action";
	...
	public static function deleteObjectifenjeu($action,$objectifEnJeu){
		$newObjectifEnjeu = array();
		foreach($action->ownObjectifenjeu as $objectifEnJeuIn){
			if(!$objectifEnJeuIn->id==$objectifEnJeu->id){
				$newObjectifEnjeu[]=$objectifEnJeuIn;
			}
		}
		$action->ownObjectifenjeu=$newObjectifEnjeu;
		R::store($action);
		return;
	}
	
	public static function addObjectifenjeu($action,$objectifEnJeu){
		$action->ownObjectifenjeu[]=$objectifEnJeu;
		R::store($action);
		return;
	}
	
	public static function getParObjectifEnJeuNonLie($objectifEnJeu){
		$actions = R::findAll(Action::$nameTable);
		$actionNonLie=array();
		foreach($actions as $action){
			$lie=false;
			foreach($action->ownObjectifenjeu as $objectifEnJeuAction){
				if($objectifEnJeu->id==$objectifEnJeuAction->id){
					$lie=true;
				}
			}
			if(!$lie){
